...
namespace Rendering
{
  namespace KaterinaPrusova
  {
    ...
    {
      //private PerlinNoiseMatejka Perlin = new PerlinNoiseMatejka(42);
      private PerlinNoise Perlin = new PerlinNoise(6, 3.0, 0.5, 3, seed: 42, baseAmplitude: 0.5);
      private PerlinNoise Perlin2 = new PerlinNoise(6, 3.0, 0.5, 3, seed: 13, baseAmplitude: 0.5);
      private PerlinNoise Perlin3 = new PerlinNoise(6, 3.0, 0.5, 3, seed: 73, baseAmplitude: 0.5);
      private PerlinNoiseMatejka PerlinMat = new PerlinNoiseMatejka(73);

      private PerlinNew PerlinNew = new PerlinNew();
      ...
      public long Apply (Intersection inter)
      {
        ...
        /*double noise1 = Perlin.GetAt(inter.TextureCoord.X, inter.TextureCoord.Y);
        double noise2 = Perlin2.GetAt(inter.TextureCoord.X, inter.TextureCoord.Y);
        double noise3 = Perlin3.GetAt(inter.TextureCoord.X, inter.TextureCoord.Y);

        double noise = Noise(inter.CoordLocal.X, inter.CoordLocal.Y, inter.CoordLocal.Z, 8);
        Console.WriteLine(noise);
        // Scale to 0-1
        noise1 = (noise1 + 1) / 2;
        noise2 = (noise2 + 1) / 2;
        ...
        inter.textureApplied = true;
        return (long)RandomStatic.numericRecipes((ulong)(noise1 * 1000000));*/
        //inter.Material.Color = new double[] { 1.0, 0.45, 0 };
        inter.SurfaceColor = new double[] { 1.0, 0.45, 0 };


        return LastAttempt(inter);
      }
      public double Noise (double x, double y, double z, double freq)
      {
        double d = 1.4;
        double x1, y1, z1;
        x1 = d * x - d * z;
        ...
        y1 = d * x1 + d * y;
        x1 = d * x1 - d * y;
        //return 8 * Perlin.GetAt(freq * x1 + 100, freq * y1, freq * z1);
        //return  PerlinMat.OctavesNoise(freq * x1 , freq * y1, freq * z1, octaves: 3, perstistence: 0.61);
        //return Perlin.GetAt(freq * x1, freq * y1, freq * z1);
        return 0.003 * PerlinNew.OctavePerlin(freq * x1 + 100, freq * y1, freq * z1, 2,2.5);
      }
      ...
      public long SomethingOld (Intersection inter)
      {
        Vector3d normalLocal = inter.NormalLocal;
        Vector3d normalObject = Vector3d.TransformVector(normalLocal, inter.LocalToObject);
        Geometry.GetTangents(ref normalObject, out Vector3d tangentUObject, out Vector3d tangentVObject);
      ...
      /*public long WithGeorgi(Intersection inter)
      {

      }*/


      public long LastAttempt (Intersection inter)
      {
        double Xlocal = inter.CoordLocal.X;
        double Ylocal = inter.CoordLocal.Y;
        double Zlocal = inter.CoordLocal.Z;
        double noise = Noise(Xlocal, Ylocal, Zlocal, 8);
        ...
        double f0 = Noise(normal[0]      ,normal[1]      ,normal[2], 8);
        double fx = Noise(normal[0]+epsilon, normal[1]      ,normal[2], 8);
        double fy = Noise(normal[0]      ,normal[1]+epsilon,normal[2], 8);
        double fz = Noise(normal[0]      ,normal[1]      ,normal[2]+ epsilon, 8);

        //Console.WriteLine(f0);

        double d = 1;
        normal[0] -= d * ((fx - f0) / epsilon);
        normal[1] -= d * ((fy - f0) / epsilon);
        normal[2] -= d * ((fz - f0) / epsilon);
        Normalize(normal);
      ...
      {
        return Matrix4d.Transpose(new Matrix4d(new Vector4d(tangentU, origX),
